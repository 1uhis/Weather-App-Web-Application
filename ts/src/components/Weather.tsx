...
const WeatherTopRightSection = ({ data }: Props) => (
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6">
      ...
      <Tile
        ...
        description={`${getWindDirection(
          Math.round(data.list[0].wind.deg)
        )}, gusts ${data.list[0].wind.gust.toFixed(1)} km/h`}
      />
      <Tile
        icon="feels"
        title="Feels like"
        info={<Degree temp={Math.round(data.list[0].main.feels_like)} />}
        description={`Feels ${
      ...
      <Tile
        ...
        description={` ${
          Math.round(data.list[0].main.pressure) < 1013 ? 'Lower' : 'Higher'
        } than standard`}
      />
      <Tile
        icon="visibility"
